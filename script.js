...
const petCategory = (category) => {
    ...
    fetch(`https://openapi.programming-hero.com/api/peddy/category/${category.id}`)
        .then(res => res.json())
        .then(data => {
            alldatapost(data.category.id);
            
            alldataapost(data.category.id);
        })
        .catch(error => console.error("Error fetching category data:", error));
};
const alldataapost = (category) => {
    const heythere = document.getElementById('heythere');
    posts.forEach(item => {
        console.log(item.category);

        const postElementt = document.createElement("div");
        ...
        heythere.append(postElementt);
    });
};

function showDetails(petName, breed, birthDate, gender, price, image) {
    const modal = document.getElementById('my_modal_1');
    modal.querySelector(".modal-box").innerHTML = `
        <h3 class="text-lg font-bold">${petName}</h3>
        <img src="${image}" alt="${petName}" class="my-2 rounded-md">
        <p class="py-2 text-gray-500">Breed: ${breed}</p>
        <p class="py-2 text-gray-500">Birth Date: ${birthDate}</p>
        <p class="py-2 text-gray-500">Gender: ${gender}</p>
        <p class="py-2 text-gray-500">Price: ${price}$</p>
...
const displaycategories = (categories) => {
    ...
    categories.forEach(item => {
        ...
        button.innerHTML = `
            <button onclick="petCategory('${item.category}')" class="btn hover:bg-[#8fb6b8] hover:rounded-3xl hover:text-white flex flex-row"><img class="w-6" src="${item.category_icon}" alt="${item.category}" />
